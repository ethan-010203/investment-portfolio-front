  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      try {
        ...
        // 浏览器禁用本地存储时仍允许当前页面正常计算。
      } finally {
        holdingsCacheReady.current = true;
      }
    });
    return () => {
      active = false;
                  ...
                  {orderedCycleEvents.map((event, index) => (
                    <article key={event.id} className="event-timeline-item">
                      ...
                      <div className="event-timeline-content">
                        ...
                        <p className="event-timeline-description">{event.asset} · {eventReasonForDisplay(event)}</p>
          ...
          <tbody>
            {visibleEvents.map((event) => (
              <tr key={event.id}>
                ...
                <td className="history-reason-cell history-event-reason">{eventReasonForDisplay(event)}</td>
              </tr>
            ))}
      ...
      <div className="history-mobile-list">
        {visibleEvents.map((event) => (
          <article key={event.id} className="history-mobile-event">
            ...
            <p>{eventReasonForDisplay(event)}</p>
          </article>
        ))}
        {visibleEvents.length === 0 && (
